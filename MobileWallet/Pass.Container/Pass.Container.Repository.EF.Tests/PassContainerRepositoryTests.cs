    ...
    public class PassContainerRepositoryTests
    {
        ...
        public void PassCrudOperationsTest()
        {
            using (var unitOfWork = new PassContainerUnitOfWork(TestHelper.DbConfig))
            {
                ...
                var pass1 = new PassEntities.Pass() {AuthToken = "123", SerialNumber = "123", Status = PassStatus.Active, PassTemplateId = pt1.PassTemplateId, PassTypeIdentifier = "123"};
                var repPass = unitOfWork.GetRepository<PassEntities.Pass>();
                repPass.Insert(pass1);

                var passField1 = new PassEntities.PassField() {Name = "PF1", PassTemplateId = pt1.PassTemplateId};
